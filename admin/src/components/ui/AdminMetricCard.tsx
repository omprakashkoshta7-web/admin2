...
export default function AdminMetricCard({
  ...
  if (isPrimary) {
    return (
      <div
        onClick={onClick}
        className={`relative rounded-[24px] p-6 overflow-visible ${onClick ? 'cursor-pointer hover:scale-[1.02] transition-transform' : ''} ${className}`.trim()}
        style={{
  ...
  return (
    <div
      onClick={onClick}
      className={`relative rounded-[24px] p-6 bg-white overflow-visible ${onClick ? 'cursor-pointer hover:scale-[1.02] transition-transform' : ''} ${className}`.trim()}
