...
export default function PlatformPage() {
  ...
  useEffect(() => {
    if (controlData) {
      const controlDataObj = controlData as any;
      setOrderIntake(controlDataObj.orderIntakeEnabled ?? true);
      setVendorIntake(controlDataObj.vendorIntakeEnabled ?? true);
      setSystemKill(controlDataObj.systemKillSwitchEnabled ?? false);
      setCityPause(controlDataObj.cityPause || {});
      
      // Transform featureFlags object to array format for display
      const flagsArray = Object.entries(controlDataObj.featureFlags || {}).map(([key, value]: [string, any]) => ({
        id: key,
        ...
        {pausedCities > 0 ? (
          <div className="space-y-2">
            {Object.entries(cityPause).filter(([_, paused]) => paused).map(([city]) => (
              <div key={city} className="flex items-center justify-between p-3 rounded-xl border border-orange-200 bg-orange-50">
                <div className="flex items-center gap-2">
