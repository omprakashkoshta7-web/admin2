...
export default function DashboardPage() {
  ...
  const { data: ordersData } = useAsync(() => dataLoaded ? getAdminOrders({ page: 1, limit: 10 }) : Promise.resolve(null), null, [dataLoaded]);
  const { data: vendorsData } = useAsync(() => dataLoaded ? getAdminVendors({ page: 1, limit: 10 }) : Promise.resolve(null), null, [dataLoaded]);
  const { data: customersData } = useAsync(() => dataLoaded ? getAdminCustomers({ page: 1, limit: 10 }) : Promise.resolve(null), null, [dataLoaded]);
  const { data: controlData } = useAsync<AdminControlStateResponse | null>(() => dataLoaded ? getAdminControlState() : Promise.resolve(null), null, [dataLoaded]);
  
  // Fetch tickets list to calculate actual counts - only after initial data loads
  const { data: ticketsListData } = useAsync(
    async () => {
      if (!dataLoaded) return { tickets: [] };
      try {
  ...
  const { data: ticketData } = useAsync(
    async () => {
      if (!dataLoaded) return { openTickets: 0, escalatedTickets: 0, criticalSLA: 0, total: 0 };
      try {
  ...
  // Fetch real alerts from backend - disabled to reduce API calls
  ...
  // Get real alerts from backend - use empty array to reduce API load
  const alerts: any[] = [];

  // Get SLA health data from backend
  const { data: slaRisksData } = useAsync(
    async () => {
      if (!dataLoaded) return { atRisk: [], totalRisks: 0 };
      try {
  ...
  const criticalAlerts = visibleAlerts.filter((a: any) => a.type === "critical");
      ...
      // Enable secondary data loading after dashboard data is available
      useEffect(() => {
        if (dashboardData && !dataLoaded) {
          // Delay secondary API calls to avoid rate limiting
      ...
      <div className="bg-white rounded-2xl overflow-hidden border-2 border-red-100" style={CS}>
        ...
        <div className="p-3 sm:p-4">
          <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-3">
            {killSwitches.map((sw, idx) => (
              <button
                key={`killswitch-${sw.id}-${idx}`}
