...
export default function DeliveryPartnerDetailPage() {
  ...
  const handleSetPayoutRate = async () => {
    try {
      ...
      const payoutRatePerOrder = parseFloat(payoutForm.payoutRatePerOrder) || 0;
      
      await setDeliveryPayoutRate(partnerId, payoutRatePerKm, payoutRatePerOrder);
      ...
      {showPayoutModal && (
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl p-6 w-full max-w-md">
            ...
            <div className="mb-4">
              <label className="block text-sm font-bold text-gray-700 mb-2">Payout Rate Per KM (₹)</label>
              <input
                type="number"
                value={payoutForm.payoutRatePerKm}
                onChange={(e) => setPayoutForm({ ...payoutForm, payoutRatePerKm: e.target.value })}
            ...
            <div className="mb-6">
              <label className="block text-sm font-bold text-gray-700 mb-2">Payout Rate Per Order (₹)</label>
              <input
                type="number"
                value={payoutForm.payoutRatePerOrder}
                onChange={(e) => setPayoutForm({ ...payoutForm, payoutRatePerOrder: e.target.value })}
                placeholder="Enter rate per order..."
                className="w-full p-3 rounded-xl border border-gray-200 focus:outline-none focus:border-gray-900 transition"
              />
            </div>
