...
export default function LedgerPage() {
  ...
  const uniqueActions: string[] = Array.from(new Set(logs.map((l: any) => l.action).filter(Boolean))) as string[];
          ...
          <table className="w-full admin-responsive-table min-w-[900px] lg:min-w-0">
            <thead>
              <tr className="border-b border-gray-100 bg-gray-50">
                {["Actor ID", "Role", "Action", "Target Type", "Target ID", "Reason", "Date & Time"].map(h => (
              ...
              {filtered.length > 0 ? (
                filtered.filter((log: any) => log && log.actorId).map((log: any) => (
                  <tr key={log._id} className="border-b border-gray-50 hover:bg-gray-50 transition">
                    ...
                    <td className="p-4">
                      <p className="text-xs text-gray-600 font-mono">{log.action || 'N/A'}</p>
                    </td>
                    <td className="p-4">
