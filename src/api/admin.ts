...
export const addTicketMessage = async (ticketId: string, content: string, _isInternal: boolean = false) => {
...
export const escalateTicket = async (ticketId: string, reason: string, _assignTo?: string) => {
...
export const setDeliveryPayoutRate = async (
  partnerId: string,
  payoutRatePerKm?: number,
  payoutRatePerOrder?: number
) => {
  const body: any = {};
  if (payoutRatePerKm !== undefined) body.payoutRatePerKm = payoutRatePerKm;
  if (payoutRatePerOrder !== undefined) body.payoutRatePerOrder = payoutRatePerOrder;
